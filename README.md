...
* 人机对战支持玩家选择黑棋先手或白棋后手。
* 人机对战支持难度选择：简单、普通。
* 简单难度只考虑让对手下一回合合法落点最少。
* AI 会优先占角，其次占边，再考虑中间位置。
* AI 会避免落子后让对手获得角或边的合法落点。
* 普通难度会综合考虑角、边、对手边角机会、对手合法落点和翻子数量。
* 页面保留本局中 AI 每次落子的推理过程，包括候选落点和评分明细。
* 棋盘和 AI 推理区域支持自适应布局：横屏水平排列，竖屏垂直排列。
* 自动跳过无棋可下的一方。
* 双方均无棋可下或棋盘已满时结束游戏。
* 黑白棋子数量统计和胜负提示。
* 悔棋、重做和重新开始。
...
* 玩家执黑先手，AI 执白后手。
* 玩家也可以选择执白后手，此时 AI 执黑并自动先走。
* 玩家只能在自己的回合点击棋盘。
* 进入 AI 回合后，页面短暂停顿并显示 AI 正在思考。
* 简单难度：AI 从所有合法落点中选择让玩家下一回合合法落点最少的位置。
* 普通难度：AI 从所有合法落点中选择一个位置，优先级为角、大于边、大于中间。
* 普通难度会模拟落子后的棋盘，如果玩家下一回合可以下角或边，该候选落点会被降权。
* 普通难度在位置价值和安全性接近时，会继续压缩玩家下一回合的合法落点数量。
...
简单难度只保留“让对手下一回合合法落点最少”的策略：

1. 取出 AI 当前所有合法落点。
2. 对每个候选落点复制一份棋盘。
3. 在复制棋盘上模拟 AI 落子和翻转。
4. 计算对手在模拟棋盘上的合法落点数量。
5. 选择让对手合法落点数量最少的候选落点。
6. 如果数量相同，则选择本步翻转棋子更多的位置。

普通难度使用一层模拟评分。目标不是当前一步翻最多棋子，而是优先获得稳定位置，并尽量避免把角或边交给对手。计算方式如下：
...
* `ReversiAI.analyzeSimpleMoves`：简单难度，只比较对手下一回合合法落点数量。
* `ReversiAI.analyzeNormalMoves`：普通难度，保留边角、机动性和翻子数量综合评分。
